let arr = [{
    ...
    "description": "Magna est magna sint sint minim fugiat non ut magna proident. Eiusmod nulla excepteur voluptate nisi quis. In dolore enim officia aliqua mollit anim sint ullamco nostrud exercitation. Non adipisicing aliqua dolore id nostrud exercitation.\r\n",
    "tags": [
        "irure",
        "mollit",
        "amet",
  ...
  let str = 'lorem';
  console.log(where(arr, str));
  console.log(tagsSearch(arr, str));

function where(arr, str) {
    let regExp = new RegExp(str, 'i');
    let arrSearch = [];
    for (let object of arr) {
        if (regExp.test(object.title) || regExp.test(object.description)) {
            arrSearch.push(object);
        }
    }
    return arrSearch;
}

function tagsSearch(arr, str) {
    let arrSearch = [];
    let regExp = new RegExp(str, 'i');
    for (let object of arr) {
        if (regExp.test(object.tags)) {
            arrSearch.push(object);
        }
    }
    if (arrSearch == 0) {
